...
class TrainerModified:
    def __init__(self, exp, args):
        ...
        if os.path.exists(self.file_name):
            logger.warning(f"Output folder exits. Removing {self.file_name} before starting this experiment")
            shutil.rmtree(self.file_name)
        
        self.metrics_json = os.path.join(self.file_name, "metrics.json")
        setup_logger(
            self.file_name,
        ...
        # copy exp file and args used in the training
        # Copy exp file
    ...
    def after_train(self):
        logger.info(
            "Training of experiment is done and the best AP is {:.3f} %".format(
                self.best_ap * 100
            )
        )
